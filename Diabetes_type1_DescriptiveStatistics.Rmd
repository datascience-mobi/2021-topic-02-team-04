...
```
## 2.1.5 all TRA expression analyis of all diabetis sets
...
## Expression values of those genes:
diab_above2med_TRA_median <- diabetes_GeneExprs[which(med_diab_TRA_median > (1.7 * median(med_diab_TRA_median)))]
...
## Genes that 1.2 times are expressed at a lower level in the last chip than in the first.
...
### 3.2.3 What are the genes that have a high variance: Important or house keeping genes?
