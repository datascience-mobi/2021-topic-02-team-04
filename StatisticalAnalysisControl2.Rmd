...
```{r}
differentialcontrol_diabetes_pancreas_fc = log2.fold.change(pancreas_GeneExprs_sub_control, diabetes_GeneExprs_sub_control)

arrange(differentialcontrol_diabetes_pancreas_fc, -log2_foldchange)
...
ggplot(data = differentialcontrol_diabetes_pancreas_fc, aes(x = log2_foldchange, y = wilcox_log10_pvalue, label = GeneSymbol, col = wilcox_trend)) + 
  ...
  labs(title = "Volcano plot (wilcox test) for control vs. control (GSE53454 & GSE59761)", 
       ...
       subtitle = "ratio = control beta cells / control PDAC cells") +
  ...
  scale_color_manual(values = c("navyblue", "grey", "orange"), 
                     labels = c("higher in PDAC cells", "not significant", "higher in beta cells")) + 
