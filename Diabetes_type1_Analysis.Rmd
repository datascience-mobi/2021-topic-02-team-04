...
diabetes_GeneExprs_specifcTRA <- combineGeneExprs_median2(diabetes_GeneExprs_sub)
...
dim(combineGeneExprs_median2(all_TRA_GeneExprs))
# 
```
# plots
## distributions
...
diabetes_df <- cbind(treatment = diabetes_microarrays[,"treatment"], 
            ...
            data.frame(t(diabetes_GeneExprs_specifcTRA)))
...
ggplot(diabetes_pca$rotation, aes(PC1, PC2, label = rownames(diabetes_pca$rotation))) + 
  geom_point() + 
  geom_label_repel(max.overlaps = 120, size = 2) +
...
ggplot(diabetes_pca$rotation[diabetes_pca_importantGenes,], aes(PC1, PC2, label = diabetes_pca_importantGenes)) + 
  geom_point() +   
  geom_label_repel(max.overlaps = 100, size = 2) +
  coord_equal(xlim = c(-0.6,0.6), ylim = c(-0.6,0.6))
...
diabetes_var = sort(apply(diabetes_GeneExprs_specifcTRA,1,var), decreasing = TRUE)
...
diabetes_CXCL11 <- data.frame(diabetes_microarrays[,c("number","treatment", "time_h")], CXCL11 = diabetes_GeneExprs_specifcTRA["CXCL11",])
...
mel = melt(t(diabetes_GeneExprs_specifcTRA[names(diabetes_var)[1:50],]))
...
diabetes_highvariance <- cbind(treatment = tr, time = ti, mel)
diabetes_highvariance
...
ggplot(diabetes_highvariance, aes(x = Var2, y = value, colour = treatment)) + 
  geom_boxplot() + 
  theme(axis.text.x = element_text(angle = 90, size = 5)) +
  xlab("Genes")
# plot boxplots of top50 high variance genes (grouped by time)
ggplot(diabetes_highvariance, aes(x = Var2, y = value, colour = time)) + 
  geom_boxplot() + 
  theme(axis.text.x = element_text(angle = 90, size = 5))+
  xlab("Genes")
```
...
## H1 (time): time does not change gene expression

diabetes_highvariance_wilcox <- t(sapply(names(diabetes_var)[1:50], function(i){
  p_treatment = wilcox.test(filter(diabetes_highvariance, treatment == "Control" & Var2 == i)$value, filter(diabetes_highvariance, treatment == "Cytok" & Var2 == i)$value)$p.value
  
  p_time = wilcox.test(filter(diabetes_highvariance, time == "1-36" & Var2 == i)$value, filter(diabetes_highvariance, time == "48-96" & Var2 == i)$value)$p.value
  
  c(p_treatment = p_treatment, p_time = p_time, 
    H1_treatment = p_treatment < 0.05, H1_time = p_time < 0.05)
}))

# p values 
diabetes_highvariance_wilcox

# which genes have a significant p value for treatment?
diabetes_highvariance_wilcox[which(diabetes_highvariance_wilcox[,"H1_treatment"] == 1),]
# nrow = 11

# which genes have a significant p value for time?
diabetes_highvariance_wilcox[which(diabetes_highvariance_wilcox[,"H1_time"] == 1),]
# nrow = 33

# which genes have a significant p value for both treatment & time?
diabetes_highvariance_wilcox[which(apply(diabetes_highvariance_wilcox[,c(3,4)],1, function(x){x[1]==1 & x[2]==1})),]
# PNLIPRP2   PABPC4    REG1A 
## not part of the diabetes_pca_importantGenes (PCA contributor )
# instead of 5% use 20% 
diabetes_highvariance_wilcox[which(apply(diabetes_highvariance_wilcox[,c(1,2)],1, function(x){x[1]<0.2 & x[2]<0.2})),]

diabetes_highvariance_wilcox[c("PNLIPRP2","REG1A","PABPC4"),]
