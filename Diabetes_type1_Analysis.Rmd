...
log2.fold.change <- function(control, test){
  ...
  trend = 1:length(t)
  trend[t < 0.05 & log2_foldchange > 0] = "up"
  trend[t < 0.05 & log2_foldchange < 0] = "down"
  trend[t >= 0.05] = "not significant"
...
ggplot(data = diabetes_fc_treatment, aes(x = log2_foldchange, y = ttest_log10_pvalue, label = GeneSymbol, col = trend)) + 
  geom_point() + 
...
ggplot(data = diabetes_fc_time, aes(x = log2_foldchange, y = ttest_log10_pvalue, label = GeneSymbol, col = trend)) + 
  geom_point() +  
  geom_hline(yintercept =  -1*log10(0.05), color = "red", linetype = "dashed") +
