...
First, we determined the difference in healthy and stressed $\beta$-cells to determine which genes are influenced by the cytokine treatment. \
To achieve this goal, we conducted a Principal Component Analysis (PCA). The optimal number of two clusters was determined by the silhouette method. Those clusters for the microarray chips were allocated by k-means. The conclusion was reached that the pancreatic-specific TRA expression overall is not considerably affected by the cytokine treatment but by the temporal component.
...
```
Moreover, a volcano plot was created reflecting the gene expression of treated versus untreated $\beta$-cells. We used a Wilcoxon rank-sum test to extract single genes that show a significant difference in their gene expression before and after cytokine treatment. \

```{r fig.cap = "Genes that are significantly higher expressed in stressed beta-cells are represented in orange. A higher expression in the normal beta-cells leads to the blue color. Genes with a p-value higher than 0.05 in the Wilcoxon rank-sum test are marked as not significant in grey.", include=TRUE, eval=TRUE}
...
```
A significantly upregulated gene in the treated group is CXCL11. This gene is known to be induced by IFN-$\gamma$ (Tokunaga et al., 2018), that was part of the treatment. This gene induces leukocytic infiltration into pancreatic tissue, which leads to $\beta$-cell destruction and ultimately, to type 1 diabetes mellitus (Burke et al., 2016).

### Difference in gene expression in PDAC data set

We followed the same steps for the PDAC cells as for the analysis of the $\beta$-cells. This time the k-means following the PCA detected two clusters: control and treatment. There is a definite divide in the overall TRA expression between these groups though no single differing gene could be identified through Wilcoxon rank-sum test.\
...
After establishing the differences of the respective control and treatment data sets, we set ourselves the goal to find genes that are related to pathways which either lead cells to uncontrolled proliferation or cell death. \
...
Meanwhile, finding genes that are highly expressed in diabetes and lowly expressed in pancreatic cancer was challenging. Some of the found genes related to pathways introducing $\beta$-cell death and therefore diabetes were ambiguous. We picked two examples to demonstrate difficulties we encountered as well as one gene which reflects the desired results. \
...
SPINK1 and REG1A can not be used to predict diabetes, because of insufficient research of the gene as well as a strong relation to pancreatic cancer. Nevertheless, we also found genes that are only associated with diabetes. \
