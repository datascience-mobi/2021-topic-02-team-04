...
colnames(differentialdifferencematrixordered) <- c("Gensymbol","Mediandifference","MedianDiabetes","MedianPancreas")
colnames(differentialdifferencematrixorderedabs) <- c("Gensymbol","Mediandifference","MedianDiabetes","MedianPancreas")
...
head(differentialdifferencematrixordered[,-1])
```
...
TSPAN8 is a member of the tetraspanin superfamily that forms TSPAN8-mediated protein complexes by interacting with themselves and other various cellular signalling molecules. Multiple such complexes build tetraspanin-enriched microdomains that mediate intracellular signal transduction. In normal physiological conditions they are involved in the regulation of diverse biological processes like cell adhesion, proliferation and differentiation. Overexpression has been linked to the progression and metastasis of several cancers by several studies by interacting with various binding partners (Kyun Heo, Sumook Lee ,2020). \
To note is also that tetraspanins are constitutive exosome components but a higher TSPAN8 expression could be found on exosomes, which could lead to a possible non-invasive cancer diagnostic – a blood serum test (Wang et al.,2013). \
