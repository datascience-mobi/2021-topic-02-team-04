...
# TRAs1
par(mai = c(0,0,0.5,0))
pie(GewebeTabelleProzent1[,1], labels = label1, col = colorRampPalette(brewer.pal(9,"Blues"))(9)[9:1], cex = 0.9, radius = 0.95, clockwise = TRUE, border = FALSE, main = "TRA expression in human tissues (atlas)")
...
# TRAs2
par(mai = c(0,0,0.5,0))
pie(GewebeTabelleProzent2[,1],labels = label2, col = colorRampPalette(brewer.pal(9,"Blues"))(9)[9:1], cex = 0.9, radius = 0.95, clockwise = TRUE, border = FALSE, main = "TRA expression in human tissues")

# TRAs3
par(mai = c(0,0,0.5,0))
pie(GewebeTabelleProzent3[,1],labels = label3, col = colorRampPalette(brewer.pal(9,"Blues"))(9)[9:1], cex = 0.9, radius = 0.95, clockwise = TRUE, border = FALSE, main = "TRA expression in human tissues (roth)")
...
# TRAs6
par(mai = c(0,0,0.5,0))
pie(GewebeTabelleProzent6[,1],labels = label6, col = colorRampPalette(brewer.pal(9,"Blues"))(9)[9:1], cex = 0.9, radius = 0.95, clockwise = TRUE, border = FALSE, main = "TRA expression in human tissues") 
...
hist(PancreasTRAs1ChromXorder,breaks=23,xlim=range(1:23),right=FALSE,main="Distribution TRAs1 across the chromosomes",xlab="Chromosom", col="lightsteelblue")
...
hist(PancreasTRAs2ChromXorder,breaks=23,xlim=range(1:23),right=FALSE,main="Distribution TRAs2 across the chromosomes",xlab="Chromosom", col="lightsteelblue")
...
hist(PancreasTRAs6ChromXorder,breaks=23,xlim=range(1:23),right=FALSE,main="Distribution TRAs6 across the chromosomes",xlab="Chromosom", col="lightsteelblue")
...
hist(PancreasTRAs4ChromXorder,breaks=20,xlim=range(1:20),right=FALSE,main="Distribution TRAs4 across the chromosomes",xlab="Chromosom", col="lightsteelblue4")
...
hist(PancreasTRAs5ChromXorder,breaks=20,xlim=range(1:20),right=FALSE,main="Distribution TRAs5 across the chromosomes",xlab="Chromosom", col="lightsteelblue4")
...
hist(diabetes_GeneExprs_combined, 
     ...
     ylab = "probability",
     main = paste("Histogram of gene expression (diabetes data set: GSE53454)"), 
     freq = FALSE)
...
hist(pancreas_GeneExprs_combined, 
     xlab = "gene expression values",
     ylab = "probability",
     main = paste("Histogram of gene expression (PDAC data set: GSE59761)"), 
     freq = FALSE)
...
pheatmap(pancreas_GeneExprs_sub,
         ...
         cluster_rows = TRUE, treeheight_row = 0,
         fontsize_row = 8, fontsize_col = 7, fontsize = 9,
         main = "TRA gene expression in PDAC cells before combining via median\ndata set: GSE59761")
...
colnames(differentialdifferencematrixorderedabs) <- c("Gensymbol","Mediandifferenceabs","MedianDiabetes","MedianPancreas")
...
log2.fold.change <- function(control, test){
  ...
  if(nrow(control) == nrow(test)){
    ...
    ratio = sapply(1:nrow(control), function(x){
      ...
      g2_m - g1_m # change of test to control (small value -> test, high value -> control)
    })
    # normalized values are already log2 transformed
    log2_foldchange = ratio
...
differential_control_diabetes_pancreas_fc = log2.fold.change(diabetes_GeneExprs_sub_control, pancreas_GeneExprs_sub_control) # ratio = control PDAC cells / beta cells
...
#### I realized that these two matrices are not exactly equal because foldchange is based on median values and not mean
...
diabetes_fc_treatment = log2.fold.change(diabetes_GeneExprs_sub_control, diabetes_GeneExprs_sub_treat)
...
ggplot(data = diabetes_fc_treatment, aes(x = log2_foldchange, y = wilcox_log10_pvalue, label = GeneSymbol, col = wilcox_trend)) + 
  ...
  geom_text_repel(size = 2, max.time = 50)
...
pancreas_fc = log2.fold.change(pancreas_GeneExprs_sub_control, pancreas_GeneExprs_sub_treat)
...
pancreas_GeneExprs_combined["CHST15",]
