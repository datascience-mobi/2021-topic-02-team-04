...
In this first part of the report, we will concentrate on the two control sets and determine if we can find highly expressed pancreatic specific antigens in the cancer cells but not in the neighboring $\beta$-cells. \
...
Our approach here was to first identify differentially expressed genes between the untreated $\beta$- and PDAC cells. For this we performed a Wilcoxon rank-sum test for each gene.
...
With the p-value set at p < 0.05 we found that 225 out of the 250 genes are significantly differentially expressed. On the basis of those genes a matrix was compiled with the median gene expression of both data sets and also the difference between them to quantify the genes. \


```{r}
differentialdifferencematrixordered[1:5,-1]
```

In this context we also calculated the log2 fold change (code zeigen für log2 fold change?) and created a volcano plot with that information. It shows the significance versus the magnitude, quantifying the difference in gene expression of a gene. \
...
We then looked for existing literature in context of an association to pancreatic cancer with those identified genes. Out of all those genes two stood out - several publications indicate TSPAN8 and GMNN as possible biomarkers for pancreatic cancer. \
