...
pancreas_pca <- prcomp(pancreas_df[,-1])
...
# all PCA contributing genes have a 2x variance higher than median variance
...
ggplot(data = pancreas_fc, aes(x = log2_foldchange, y = wilcox_log10_pvalue, label = GeneSymbol)) + 
  geom_point() + 
  geom_hline(yintercept =  -1*log10(0.05), color = "red", linetype = "dashed") + 
  labs(title = "Vulcano plot (wilcoxon rank sum test)", 
       x = "log2 fold change", y = "-log10 (p-value)") +
  xlim(c(-max(abs(pancreas_fc$log2_foldchange)),
         max(abs(pancreas_fc$log2_foldchange))))

ggplot(data = pancreas_fc, aes(x = log2_foldchange, y = ttest_log10_pvalue, label = GeneSymbol, col = trend)) + 
  geom_point() + 
  geom_hline(yintercept =  -1*log10(0.05), color = "red", linetype = "dashed") + 
  labs(title = "Vulcano plot (t test)", 
       x = "log2 fold change", y = "-log10 (p-value)") +
  xlim(c(-max(abs(pancreas_fc$log2_foldchange)),
         max(abs(pancreas_fc$log2_foldchange))))+ 
