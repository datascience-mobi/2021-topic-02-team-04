...
pancreas_GeneExprs_specifcTRA <- combineGeneExprs_median2(pancreas_GeneExprs_sub)
...
boxplot(t(pancreas_GeneExprs_specifcTRA),
         col = rainbow(250),
         main="",
         horizontal = TRUE)
...
pancreas_df <-cbind(treatment = pancreas_microarrays[,"siRNA"],
            data.frame(t(pancreas_GeneExprs_specifcTRA)))
# PCA on the TRA expressions
pancreas_pca <- prcomp(pancreas_df[,-c(1,2)])
# elbow plot 
...
autoplot(prcomp(pancreas_df[,-c(1,2)]), data = pancreas_df, colour = "treatment", main = "Principal Component Analysis on beta cell data set", xlim = c(-0.7,0.7), ylim = c(-0.7,0.7), asp = 1)
autoplot(prcomp(pancreas_df[,-c(1,2)]), data = pancreas_df, colour = "treatment", loadings = TRUE, loadings.label = TRUE, loadings.label.size = 3, main = "Principal Component Analysis on beta cell data set", xlim = c(-0.7,0.7), ylim = c(-0.7,0.7), asp = 1)

autoplot(kmeans(pancreas_pca$x,2), data = pancreas_pca$x, frame = TRUE,  xlim = c(-0.7,0.7), ylim = c(-0.7,0.7), asp = 1, main = "kmeans clustering (k=2) on beta cell data set")
...
pancreas_var = sort(apply(pancreas_GeneExprs_specifcTRA,1,var), decreasing = TRUE)
...
# all PCA contributing genes have a 2x variance higher than  variance
names(pancreas_var)[1:9] %in% pancreas_pca_importantGenes
# all top 9 high variance genes contribute strongly in PCA
# (there are genes with high variance that don't really contribute to PC1/PC2)
```

```{r}
# look at gene with highest variance (CHST4)
pancreas_CHST4 <- data.frame(pancreas_microarrays[,c("number","siRNA")], CHST4 = pancreas_GeneExprs_specifcTRA["CHST4",])
...
mel = melt(t(pancreas_GeneExprs_specifcTRA[names(pancreas_var)[1:10],]))
...
pancreas_highvariance <- cbind(treatment = tr, mel)
remove(tr,mel)

# plot boxplots of top10 high variance genes (grouped by treatment)
ggplot(pancreas_highvariance, aes(x = Var2, y = value, colour = treatment)) + 
  geom_boxplot() + 
...
## H1_.05 -> alpha = 5% (p < alpha)
## H1_.15 -> alpha = 15% (p < alpha)

pancreas_highvariance_wilcox <- t(sapply(names(var)[1:10], function(i){
  p_treatment = wilcox.test(filter(pancreas_highvariance, treatment == "NC" & Var2 == i)$value, filter(pancreas_highvariance, treatment == "TBL1" & Var2 == i)$value)$p.value
  
  c(p_treatment = p_treatment, H1_.05 = p_treatment < 0.05, H1_.15 = p_treatment < 0.15)
}))

# p values
pancreas_highvariance_wilcox

rownames(pancreas_highvariance_wilcox)[pancreas_highvariance_wilcox[,"H1_.15"]==1] 
# each gene is contributing to PCA (part of pancreas_pca_importantGenes)

autoplot(pancreas_GeneExprs_specifcTRA[c("CHST4", "PABPC4", "ECI2", "TMEM97", "ANXA4", "NEURL3", "MKNK1", "MED24"),], xlab = "Microarrays", ylab = "Genes")
```
